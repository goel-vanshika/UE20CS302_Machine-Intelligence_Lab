...
class KMeansClustering:
    ...
    def fit(self, data):
        ...
        if data.shape[0] < self.n_cluster:
            raise ValueError('Number of clusters is grater than number of datapoints')
    ...
    def e_step(self, data):
        ...
        for k in range(self.n_cluster):
            # Calucluate Euclidean distance
            squared = np.square(data - self.centroids[k, :])
            distance[:, k] = np.sqrt(np.sum(squared, axis=1))

        # Take the argmin of the distances
    ...
    def m_step(self, data, cluster_assgn):
        ...
        """
        for k in range(self.n_cluster):
            # compute centroids based on the mean
            self.centroids[k,:] = np.mean(data[cluster_assgn == k, :], axis=0)
        return
